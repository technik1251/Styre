...
window.rWiz = function() {
    ...
    let userBox = isLogged ? 
        `<div style="background:rgba(34,197,94,0.1); border:1px solid var(--success); padding:15px; border-radius:12px; text-align:center; margin-bottom:25px;">
            ...
            <span style="font-size:0.8rem; color:var(--muted);">Zalogowano jako: ${firebase.auth().currentUser.displayName}</span>
        </div>` : 
    ...
    <div id="w-home" class="wiz-screen">
        <div class="w-title">Budżet Rodzinny</div>
...
window.hFin = function() { 
    ...
    window.db.role = 'home'; window.db.tab = 'dash'; window.db.init = true; if(window.save) window.save(); window.render(); 
}
