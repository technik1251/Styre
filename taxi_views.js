...
window.rDrv = function() {
    try {
        ...
        let navStyle = '<style>' +
            '.float-nav { position:fixed; bottom:20px; left:5%; width:90%; background:rgba(15,15,20,0.8); backdrop-filter:blur(20px); -webkit-backdrop-filter:blur(20px); border:none; border-radius:30px; display:flex; justify-content:space-around; align-items:center; padding:12px 6px; box-shadow:0 25px 50px rgba(0,0,0,0.85); z-index:9999; }' +
            '.f-item { display:flex; flex-direction:column; align-items:center; justify-content:center; color:rgba(255,255,255,0.4); font-size:0.6rem; font-weight:800; text-transform:uppercase; letter-spacing:0.8px; flex:1; cursor:pointer; transition:all 0.4s cubic-bezier(0.4, 0, 0.2, 1); }' +
            '.f-item.act { color:#fff; }' +
            '.f-icon { font-size:1.6rem; margin-bottom:5px; filter:grayscale(100%) opacity(0.4); transition:all 0.4s cubic-bezier(0.4, 0, 0.2, 1); transform:scale(0.85); }' +
            '.f-item.act .f-icon { filter:grayscale(0%) opacity(1); transform:scale(1.2) translateY(-3px); }' +
            '.f-dot { width:5px; height:5px; border-radius:50%; background:transparent; margin-top:3px; transition:all 0.4s ease; }' +
            '</style>';

        // Dolny pasek nawigacji (Pływająca Wyspa bez ramek)
