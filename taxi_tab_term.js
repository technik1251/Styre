// ==========================================
// PLIK: taxi_tab_term.js - Terminal Premium V8 (Pełna Automatyzacja Netto, Zero Lag)
// ==========================================
...
// Automatyczne pobieranie wszystkich zadeklarowanych kosztów z Ustawień (ZUS, Wynajem, Baza)
window.getFixedDailyCosts = function() {
    let c = window.db.drv.cfg || {};
    let rent = parseFloat(c.carRent || c.rent || c.costRent) || 0; // zakładamy format tygodniowy
    let zus = parseFloat(c.zus || c.costZus) || 0; // zakładamy format miesięczny
    let eFix = (c.eType === 'fix') ? (parseFloat(c.eFix) || 0) : 0; // opłata partnera tygodniowa
    let fixedOther = parseFloat(c.fixedDaily || c.otherFix) || 0; // inne stałe dzienne
    
    let dailyCosts = (rent / 7) + (zus / 30) + (eFix / 7) + fixedOther;
    return dailyCosts;
};
...
window.dStartS = function() {
    ...
    // ZAMROŻENIE KOSZTÓW: Pobieramy koszty dokładnie w momencie startu zmiany
...
window.dCancelShift = function() {
    if(confirm('Czy na pewno chcesz anulować bieżącą zmianę? Zebrane dziś dane i dystans przepadną bezpowrotnie.')) {
...
// --- NIEZAWODNY GLOBAL TRACKER (ZAPIS W TLE) ---
...
window.dSetGoal = function() {
    ...
    let modeName = isNetto ? 'NETTO (Operacyjne)' : 'BRUTTO (Utarg)';
...
window.submitPremiumEndShift = function() {
    ...
    let fixedCosts = window.getFixedDailyCosts(); // Pobiera z parsowania Ustawień na koniec
