...
window.rHomeGoals = function(h, t, nav, hdr) {
    ...
    if (activeLoans.length === 0) { 
        ...
        let mappedLoans = activeLoans.map(l => {
            ...
            let isConverted = l.isConverted || (totInst > 1);
            ...
            if (isKredyt) {
                ...
                if (!isConverted && dniOdZakupu <= 31) {
                    ...
                    savingsMsg = `Spłacając dziś, nie ponosisz żadnych kosztów! 🛍️`;
                    savingsColor = 'var(--success)';
                    
                    let timeCol = pozostaloDniDarmowych >= 0 ? 'var(--success)' : 'var(--danger)';
                    let timeMsg = pozostaloDniDarmowych >= 0 ? `Masz jeszcze ${pozostaloDniDarmowych} dni na darmową spłatę! 🛡️` : 'Czas na darmową spłatę minął! ⚠️';
                    ...
                    if(oszczednoscDnia > 0) {
                        savingsMsg = `Spłacając dziś całość, unikasz części opłat! 🛍️`;
                        savingsColor = 'var(--info)';
                    }
                    bnplAlert = `<div style="color:var(--warning); font-size:0.75rem; font-weight:bold; margin-top:5px;">Harmonogram ratalny w toku. 📉</div>`;
                }
            }
            ...
            if(isBNPL) {
                ...
                if (prowizja > 0) {
                    let pColor = (isFreePeriod && !isConverted) ? 'var(--muted)' : 'var(--warning)';
                    let pText = (isFreePeriod && !isConverted) ? `(Ominiesz płacąc do ${daysLeft} dni)` : `Naliczono / Rozłożono na raty`;
    ...
    let piggyHtml = window.db.home.piggy.map(p => {
        ...
        let pColor = 'var(--info)';
        if(pct >= 100) pColor = '#eab308'; 
        else if(pct >= 75) pColor = 'var(--success)';
        else if(pct >= 50) pColor = 'var(--info)';
        else if(pct >= 25) pColor = 'var(--warning)';
        else pColor = 'var(--danger)';

        let deadlineHtml = ''; 
        if(p.deadline) { 
            let dLine = new Date(p.deadline); 
            let diffDays = Math.ceil((dLine - dzis) / (1000 * 60 * 60 * 24)); 
            if(diffDays > 0) { 
                let toSave = target - saved;
                if(toSave < 0) toSave = 0;
                let perMonth = toSave / (diffDays / 30.4); 
                let perDay = toSave / diffDays;
                
                deadlineHtml = `
                <div style="font-size:0.75rem; color:var(--muted); margin-bottom:10px; background:rgba(255,255,255,0.05); padding:10px; border-radius:8px; border-left:3px solid ${pColor};">
                    Zostało <strong>${diffDays} dni</strong> do celu.<br>
                    💡 Wystarczy, że odłożysz <strong style="color:${pColor}; font-size:0.9rem;">${Number(perDay).toFixed(2)} zł dziennie</strong> (lub ok. ${Number(perMonth).toFixed(0)} zł/m-c).
                </div>`; 
            } else if (pct >= 100) {
                deadlineHtml = `<div style="font-size:0.8rem; color:#eab308; margin-bottom:10px; font-weight:bold; text-align:center; padding:5px;">🎉 Cel osiągnięty! Gratulacje! 🎉</div>`;
            } else { 
                deadlineHtml = `<div style="font-size:0.75rem; color:var(--danger); margin-bottom:10px;">Czas na realizację minął (${p.deadline})</div>`; 
            } 
        }

        let historyTrans = window.db.home.trans.filter(x => x.piggyId == p.id).sort((a,b) => new Date(b.rD) - new Date(a.rD));
        let histHtml = historyTrans.map(tx => {
            let isDep = tx.piggyAction === 'deposit';
            return `
            <div style="display:flex; justify-content:space-between; font-size:0.75rem; padding:6px 0; border-bottom:1px solid rgba(255,255,255,0.05);">
                <span style="color:var(--muted);">${tx.dt}</span>
                <strong style="color:${isDep?'var(--success)':'var(--danger)'}">${isDep?'+':'-'}${Number(tx.amount||tx.v).toFixed(2)} zł</strong>
            </div>`;
        }).join('') || '<div style="font-size:0.7rem; color:var(--muted); text-align:center;">Brak historii wpłat.</div>';

        return `
        <div class="panel" style="padding:15px; border-left:4px solid ${pColor}; background:linear-gradient(145deg, #18181b, #09090b); margin-bottom:15px; border-radius:16px;">
            <div style="display:flex; justify-content:space-between; margin-bottom:10px; align-items:center;">
                <div style="display:flex; align-items:center; gap:10px;">
                    <div style="font-size:1.8rem; background:rgba(255,255,255,0.05); width:40px; height:40px; border-radius:10px; display:flex; justify-content:center; align-items:center;">${p.icon || '🎯'}</div>
            ...
            <div style="width:100%; height:12px; background:rgba(0,0,0,0.5); border-radius:6px; overflow:hidden; margin-bottom:12px; position:relative;">
                ...
                ${pct >= 25 ? `<div style="position:absolute; left:25%; top:0; bottom:0; width:1px; background:rgba(255,255,255,0.3);"></div>` : ''}
                ${pct >= 50 ? `<div style="position:absolute; left:50%; top:0; bottom:0; width:1px; background:rgba(255,255,255,0.3);"></div>` : ''}
                ${pct >= 75 ? `<div style="position:absolute; left:75%; top:0; bottom:0; width:1px; background:rgba(255,255,255,0.3);"></div>` : ''}
            </div>
            
            <div style="text-align:center; margin-bottom:10px;">
                <span onclick="let el=document.getElementById('phist_${p.id}'); let txt=this; if(el.style.display==='none'){el.style.display='block'; txt.innerHTML='🔼 Zwiń historię';}else{el.style.display='none'; txt.innerHTML='🔽 Rozwiń historię';}" style="color:var(--muted); font-size:0.7rem; cursor:pointer; text-decoration:underline;">🔽 Rozwiń historię</span>
            </div>
            <div id="phist_${p.id}" style="display:none; margin-bottom:12px; max-height:100px; overflow-y:auto; padding-right:5px; background:rgba(0,0,0,0.2); border-radius:8px; padding:10px;">${histHtml}</div>

            <div style="display:flex; gap:10px;">
                <button style="background:rgba(34,197,94,0.15); color:var(--success); border:1px solid rgba(34,197,94,0.3); border-radius:10px; padding:12px 10px; flex:2; font-weight:bold; cursor:pointer; font-size:0.8rem;" onclick="window.hAddFundsPiggy('${p.id}')">💸 WPŁAĆ ŚRODKI</button>
                <button style="background:rgba(239,68,68,0.15); color:var(--danger); border:1px solid rgba(239,68,68,0.3); border-radius:10px; padding:12px 10px; flex:1; font-weight:bold; cursor:pointer; font-size:0.8rem;" onclick="window.hWithdrawFundsPiggy('${p.id}')">🔨 ROZBIJ</button>
            </div>
        </div>`;
    ...
    if(appContainer) {
        ...
        <div style="padding: 10px 15px; margin-bottom:20px;">
            <div style="text-align:center; margin-bottom:15px;">
                <button class="btn" style="background:linear-gradient(135deg, var(--success), #16a34a); color:#fff; border-radius:12px; font-weight:900; box-shadow:0 4px 20px rgba(34,197,94,0.4); width:auto; padding:12px 25px; font-size:0.9rem;" onclick="window.hOpenPiggyModal()">+ DODAJ NOWĄ SKARBONKĘ</button>
            </div>
            ${piggyHtml}
        </div>
        ${proBanner}
