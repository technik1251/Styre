...
window.rDrvSet = function(d, t, nav, hdr) {
    try {
        ...
        let alertCode = "if(window.sysAlert) window.sysAlert('Chmura i Profile PRO', 'W wersji PRO Twoje dane będą bezpiecznie synchronizowane w chmurze! Będziesz mógł stworzyć osobne profile. ☁️🚀', 'info')";
        
        let proBannerSettings = '<div class="pro-teaser-panel" style="margin-bottom: 25px; padding: 20px; background: linear-gradient(135deg, #130a1c 0%, #000000 100%); border: 1px solid rgba(217, 70, 239, 0.3); border-radius: 24px; position: relative; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.5); cursor: pointer; transition: transform 0.2s;" onclick="' + alertCode + '">' +
        ...
        // 1. TARYFIKATOR
        html += '<div class="panel" style="padding:0; border-radius:24px; margin-bottom:15px; overflow:hidden; border:1px solid rgba(255,255,255,0.05); background:linear-gradient(145deg, #18181b, #09090b); box-shadow:0 10px 30px rgba(0,0,0,0.4);">' +
            '<div onclick="window.toggleAccordion(\'acc-tar\')" style="padding:20px; display:flex; justify-content:space-between; align-items:center; cursor:pointer; background:rgba(217, 70, 239, 0.05);">' +
                '<strong style="color:#d946ef; font-size:0.85rem; letter-spacing:1px; text-transform:uppercase; display:flex; align-items:center;"><span style="font-size:1.5rem; margin-right:12px; text-shadow:0 0 8px rgba(217,70,239,0.4);">🧮</span> Ustawienia Wyceny</strong>' +
        ...
        html += '<div class="panel" style="padding:0; border-radius:24px; margin-bottom:15px; overflow:hidden; border:1px solid rgba(255,255,255,0.05); background:linear-gradient(145deg, #18181b, #09090b); box-shadow:0 10px 30px rgba(0,0,0,0.4);">' +
            '<div onclick="window.toggleAccordion(\'acc-pers\')" style="padding:20px; display:flex; justify-content:space-between; align-items:center; cursor:pointer; background:rgba(16, 185, 129, 0.05);">' +
                '<strong style="color:#10b981; font-size:0.85rem; letter-spacing:1px; text-transform:uppercase; display:flex; align-items:center;"><span style="font-size:1.5rem; margin-right:12px; text-shadow:0 0 8px rgba(16,185,129,0.4);">👤</span> Personalizacja</strong>' +
            ...
            '<div id="acc-pers" style="display:none; padding:20px; border-top:1px solid rgba(255,255,255,0.05);">' +
                ...
                '<div class="inp-group" style="margin:0;">' +
                    '<label style="'+lblStyle+'">Domyślne Miasto (Mapy)</label>' +
        ...
        html += '<div class="panel" style="padding:0; border-radius:24px; margin-bottom:15px; overflow:hidden; border:1px solid rgba(255,255,255,0.05); background:linear-gradient(145deg, #18181b, #09090b); box-shadow:0 10px 30px rgba(0,0,0,0.4);">' +
            '<div onclick="window.toggleAccordion(\'acc-fuel\')" style="padding:20px; display:flex; justify-content:space-between; align-items:center; cursor:pointer; background:rgba(245, 158, 11, 0.05);">' +
                '<strong style="color:#f59e0b; font-size:0.85rem; letter-spacing:1px; text-transform:uppercase; display:flex; align-items:center;"><span style="font-size:1.5rem; margin-right:12px; text-shadow:0 0 8px rgba(245,158,11,0.4);">⛽</span> Koszty Paliwa</strong>' +
            ...
            '<div id="acc-fuel" style="display:none; padding:20px; border-top:1px solid rgba(255,255,255,0.05);">' +
                '<div class="inp-group" style="margin-bottom:20px; border-bottom:1px dashed rgba(255,255,255,0.1); padding-bottom:20px;">' +
                    '<label style="'+lblStyle+' color:#f59e0b;">Zaznacz rodzaje zasilania</label>' +
                    '<div style="display:flex; gap:8px; flex-wrap:wrap; margin-top:10px;">' +
                        '<label style="display:flex; align-items:center; gap:8px; background:rgba(255,255,255,0.03); padding:12px 16px; border-radius:12px; border:1px solid rgba(255,255,255,0.08); cursor:pointer; flex:1; min-width:40%; font-size:0.85rem; font-weight:700;">' +
                            '<input type="checkbox" id="cb-ftype-pb" value="pb" '+cPb+' onchange="window.toggleManualFuelBoxes && window.toggleManualFuelBoxes()" style="accent-color:#f59e0b; width:18px; height:18px;"> Benzyna' +
                        '</label>' +
                        '<label style="display:flex; align-items:center; gap:8px; background:rgba(255,255,255,0.03); padding:12px 16px; border-radius:12px; border:1px solid rgba(255,255,255,0.08); cursor:pointer; flex:1; min-width:40%; font-size:0.85rem; font-weight:700;">' +
                            '<input type="checkbox" id="cb-ftype-on" value="on" '+cOn+' onchange="window.toggleManualFuelBoxes && window.toggleManualFuelBoxes()" style="accent-color:#f59e0b; width:18px; height:18px;"> Diesel' +
                        '</label>' +
                        '<label style="display:flex; align-items:center; gap:8px; background:rgba(255,255,255,0.03); padding:12px 16px; border-radius:12px; border:1px solid rgba(255,255,255,0.08); cursor:pointer; flex:1; min-width:40%; font-size:0.85rem; font-weight:700;">' +
                            '<input type="checkbox" id="cb-ftype-lpg" value="lpg" '+cLpg+' onchange="window.toggleManualFuelBoxes && window.toggleManualFuelBoxes()" style="accent-color:#f59e0b; width:18px; height:18px;"> Gaz (LPG)' +
                        '</label>' +
                        '<label style="display:flex; align-items:center; gap:8px; background:rgba(255,255,255,0.03); padding:12px 16px; border-radius:12px; border:1px solid rgba(255,255,255,0.08); cursor:pointer; flex:1; min-width:40%; font-size:0.85rem; font-weight:700;">' +
                            '<input type="checkbox" id="cb-ftype-ev" value="ev" '+cEv+' onchange="window.toggleManualFuelBoxes && window.toggleManualFuelBoxes()" style="accent-color:#0ea5e9; width:18px; height:18px;"> Prąd (EV)' +
                        '</label>' +
                    '</div>' +
                '</div>' +
                '<div class="inp-group" style="margin-bottom:10px;">' +
                    ...
                    '<select id="us-fuel-src" onchange="window.toggleManualFuelBoxes && window.toggleManualFuelBoxes()" style="'+inpStyle+' border-color:rgba(245,158,11,0.3);">' +
                        '<option value="garage" '+selGar+'>Dziennik Garażu (Dokładne)</option>' +
                        '<option value="manual" '+selMan+'>Z ryczałtu (Wpisane poniżej)</option>' +
                    '</select>' +
                '</div>' +
                ...
                '<div id="manual-fuel-wrapper" style="display:'+(fuelSource==='manual'?'block':'none')+'; margin-top:20px; border-top:1px dashed rgba(255,255,255,0.1); padding-top:20px;">' +
                    '<p style="font-size:0.75rem; color:var(--muted); text-align:center; margin-bottom:15px; font-weight:600;">Podaj średnie parametry. Wyliczymy koszt 1 KM.</p>' +
                    
                    '<div id="mf-box-pb" style="display:'+(cPb?'block':'none')+'; margin-bottom:15px; background:rgba(0,0,0,0.3); padding:15px; border-radius:16px; border:1px solid rgba(255,255,255,0.05);">' +
                        '<div style="color:#f59e0b; font-size:0.75rem; font-weight:800; margin-bottom:10px; text-transform:uppercase;">⛽ Benzyna</div>' +
                        '<div class="inp-row" style="margin:0; gap:12px;">' +
                            '<div class="inp-group" style="margin:0; flex:1;"><label style="'+lblStyle+'">L/100km</label><input type="number" step="0.1" id="mf-c-pb" value="'+mF.pb.c+'" style="'+inpStyle+' text-align:center;"></div>' +
                            '<div class="inp-group" style="margin:0; flex:1;"><label style="'+lblStyle+'">Cena ZŁ/L</label><input type="number" step="0.01" id="mf-p-pb" value="'+mF.pb.p+'" style="'+inpStyle+' text-align:center;"></div>' +
                        '</div>' +
                    '</div>' +
                    
                    '<div id="mf-box-on" style="display:'+(cOn?'block':'none')+'; margin-bottom:15px; background:rgba(0,0,0,0.3); padding:15px; border-radius:16px; border:1px solid rgba(255,255,255,0.05);">' +
                        '<div style="color:#f59e0b; font-size:0.75rem; font-weight:800; margin-bottom:10px; text-transform:uppercase;">⛽ Diesel</div>' +
                        '<div class="inp-row" style="margin:0; gap:12px;">' +
                            '<div class="inp-group" style="margin:0; flex:1;"><label style="'+lblStyle+'">L/100km</label><input type="number" step="0.1" id="mf-c-on" value="'+mF.on.c+'" style="'+inpStyle+' text-align:center;"></div>' +
                            '<div class="inp-group" style="margin:0; flex:1;"><label style="'+lblStyle+'">Cena ZŁ/L</label><input type="number" step="0.01" id="mf-p-on" value="'+mF.on.p+'" style="'+inpStyle+' text-align:center;"></div>' +
                        '</div>' +
                    '</div>' +
                    
                    '<div id="mf-box-lpg" style="display:'+(cLpg?'block':'none')+'; margin-bottom:15px; background:rgba(0,0,0,0.3); padding:15px; border-radius:16px; border:1px solid rgba(255,255,255,0.05);">' +
                        '<div style="color:#f59e0b; font-size:0.75rem; font-weight:800; margin-bottom:10px; text-transform:uppercase;">⛽ Gaz LPG</div>' +
                        '<div class="inp-row" style="margin:0; gap:12px;">' +
                            '<div class="inp-group" style="margin:0; flex:1;"><label style="'+lblStyle+'">L/100km</label><input type="number" step="0.1" id="mf-c-lpg" value="'+mF.lpg.c+'" style="'+inpStyle+' text-align:center;"></div>' +
                            '<div class="inp-group" style="margin:0; flex:1;"><label style="'+lblStyle+'">Cena ZŁ/L</label><input type="number" step="0.01" id="mf-p-lpg" value="'+mF.lpg.p+'" style="'+inpStyle+' text-align:center;"></div>' +
                        '</div>' +
                    '</div>' +
                    
                    '<div id="mf-box-ev" style="display:'+(cEv?'block':'none')+'; margin-bottom:15px; background:rgba(0,0,0,0.3); padding:15px; border-radius:16px; border:1px solid rgba(255,255,255,0.05);">' +
                        '<div style="color:#0ea5e9; font-size:0.75rem; font-weight:800; margin-bottom:10px; text-transform:uppercase;">⚡ Prąd (EV)</div>' +
                        '<div class="inp-row" style="margin:0; gap:12px;">' +
                            '<div class="inp-group" style="margin:0; flex:1;"><label style="'+lblStyle+'">kWh/100km</label><input type="number" step="0.1" id="mf-c-ev" value="'+mF.ev.c+'" style="'+inpStyle+' text-align:center;"></div>' +
                            '<div class="inp-group" style="margin:0; flex:1;"><label style="'+lblStyle+'">Cena ZŁ/kWh</label><input type="number" step="0.01" id="mf-p-ev" value="'+mF.ev.p+'" style="'+inpStyle+' text-align:center;"></div>' +
                        '</div>' +
                    '</div>' +
                '</div>' +
            '</div>' +
        ...
        html += '<div class="panel" style="padding:0; border-radius:24px; margin-bottom:15px; overflow:hidden; border:1px solid rgba(255,255,255,0.05); background:linear-gradient(145deg, #18181b, #09090b); box-shadow:0 10px 30px rgba(0,0,0,0.4);">' +
            ...
            '<div id="acc-car" style="display:none; padding:20px; border-top:1px solid rgba(255,255,255,0.05);">' +
                ...
                '<div class="inp-row" style="margin-bottom:5px; gap:12px;">' +
                    '<div class="inp-group" style="margin:0; flex:2;"><label style="'+lblStyle+'">ZUS Ubezp. (zł)</label><input type="number" id="us-ic" value="'+insC+'" style="'+inpStyle+'"></div>' +
        ...
        let selFlat = empType === 'flat' ? 'selected' : '';
        let selPct = empType === 'pct' ? 'selected' : '';
        
        html += '<div class="panel" style="padding:0; border-radius:24px; margin-bottom:25px; overflow:hidden; border:1px solid rgba(255,255,255,0.05); background:linear-gradient(145deg, #18181b, #09090b); box-shadow:0 10px 30px rgba(0,0,0,0.4);">' +
            '<div onclick="window.toggleAccordion(\'acc-tax\')" style="padding:20px; display:flex; justify-content:space-between; align-items:center; cursor:pointer; background:rgba(14,165,233,0.05);">' +
                '<strong style="color:#0ea5e9; font-size:0.85rem; letter-spacing:1px; text-transform:uppercase; display:flex; align-items:center;"><span style="font-size:1.5rem; margin-right:12px; text-shadow:0 0 8px rgba(14,165,233,0.4);">⚖️</span> Podatki i Prowizje</strong>' +
            ...
            '<div id="acc-tax" style="display:none; padding:20px; border-top:1px solid rgba(255,255,255,0.05);">' +
                '<div class="inp-row" style="margin-bottom:15px; gap:12px;">' +
                    ...
                    '<div class="inp-group" style="margin:0; flex:1;"><label style="'+lblStyle+'">Typ Rozliczenia</label><select id="us-etype" onchange="window.dCheckEPct && window.dCheckEPct()" style="'+inpStyle+'">' +
                        '<option value="flat" '+selFlat+'>Stała (ZUS/Umowa)</option>' +
                        '<option value="pct" '+selPct+'>% Utargu</option>' +
                    '</select></div>' +
                '</div>' +
                '<div id="us-ep-box" style="margin-bottom:20px;">';
                
                if(empType === 'pct') {
                    html += '<div class="inp-group" style="margin:0;"><label style="'+lblStyle+'">Prowizja partnera (%)</label><input type="number" id="us-epct" value="'+empPct+'" style="'+inpStyle+'"></div>';
                } else {
                    let epWeek = empPer === 'week' ? 'selected' : '';
                    let epMon = empPer === 'month' ? 'selected' : '';
                    html += '<div class="inp-row" style="margin:0; gap:12px;">' +
                        ...
                        '<div class="inp-group" style="margin:0; flex:1;"><label style="'+lblStyle+'">Okres</label><select id="us-e-period" style="'+inpStyle+'"><option value="week" '+epWeek+'>Tyg</option><option value="month" '+epMon+'>M-c</option></select></div>' +
                    '</div>';
                }
        ...
        html += '</div>' +
                ...
                '</div>' +
                '<div class="inp-group" style="margin:0;"><label style="'+lblStyle+'">Prowizja Voucherów (%)</label><input type="number" id="us-vf" value="'+vouchF+'" placeholder="0" step="0.1" style="'+inpStyle+'"></div>' +
            '</div>' +
        '</div>';
        ...
        html += '<div class="panel" style="padding:35px 20px; border-radius:24px; margin-bottom:20px; text-align:center; border:1px solid rgba(245,158,11,0.4); background:linear-gradient(145deg, #2a1505 0%, #090500 100%); box-shadow:0 15px 40px rgba(245,158,11,0.15); position:relative; overflow:hidden;">' +
            '<div style="font-size:4rem; margin-bottom:15px; text-shadow: 0 4px 15px rgba(245,158,11,0.6); animation: pulse 2s infinite;">☕</div>' +
        ...
        html += '<div style="display:flex; gap:12px; padding: 0 15px 25px 15px;">' +
        ...
        html += '<div style="text-align:center; padding: 20px 0; padding-bottom: 80px;">' +
            '<img src="icon-512.png" style="width:50px;height:50px; opacity:0.1; mix-blend-mode:luminosity;" class="float-icon" alt="StyreOS">' +
        ...
        html += '<input type="file" id="h-import-file" style="display:none;" onchange="if(window.dImport) window.dImport(event)">' +
