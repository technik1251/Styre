    ...
    try {
        ...
        if(t === 'stats') {
            ...
            if(appContainer) {
                ...
                '<div class="mode-switch" style="margin:15px; border-radius:14px; padding:6px; background:rgba(0,0,0,0.4); border:1px solid rgba(255,255,255,0.05);">' +
                ...
                '</div>' +
                customDatesHtml +
                '<div style="display:flex; justify-content:center; gap:8px; margin-bottom:20px; padding:0 15px;">' +
                    '<button class="chip '+(window.db.drv.showFixed?'active':'')+'" style="font-size:0.65rem; padding:8px 16px; border-radius:20px; font-weight:800; background:'+(window.db.drv.showFixed?'rgba(255,255,255,0.1)':'transparent')+'; border:1px solid rgba(255,255,255,0.1); color:'+(window.db.drv.showFixed?'#fff':'var(--muted)')+';" onclick="window.db.drv.showFixed=true; window.render()">Koszty pełne</button>' +
