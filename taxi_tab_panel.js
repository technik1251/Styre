...
window.rDrvPanel = function(d, t, nav, hdr) {
    try {
        ...
        if(t === 'term') {
            ...
            let chipStyle = 'border-radius:20px; font-weight:700; padding:10px 18px; font-size:0.75rem; letter-spacing:0.5px; border:1px solid rgba(255,255,255,0.05); transition:all 0.2s;';
            let chipActBlue = 'background:rgba(14,165,233,0.15); color:#0ea5e9; border-color:rgba(14,165,233,0.4); box-shadow:0 4px 12px rgba(14,165,233,0.1);';
            let chipActGreen = 'background:rgba(16,185,129,0.15); color:#10b981; border-color:rgba(16,185,129,0.4); box-shadow:0 4px 12px rgba(16,185,129,0.1);';
            
            let ch1 = '';
            if(d.plat === 'apps') {
                ch1 = '<div class="chip '+(window.dTSrc==='Uber'?'active':'')+'" style="'+chipStyle+(window.dTSrc==='Uber'?chipActBlue:'background:rgba(255,255,255,0.03); color:var(--muted);')+'" onclick="window.dTC(\'s\',\'Uber\')">Uber</div>' +
                      '<div class="chip '+(window.dTSrc==='Bolt'?'active':'')+'" style="'+chipStyle+(window.dTSrc==='Bolt'?chipActBlue:'background:rgba(255,255,255,0.03); color:var(--muted);')+'" onclick="window.dTC(\'s\',\'Bolt\')">Bolt</div>' +
                      '<div class="chip '+(window.dTSrc==='FreeNow'?'active':'')+'" style="'+chipStyle+(window.dTSrc==='FreeNow'?chipActBlue:'background:rgba(255,255,255,0.03); color:var(--muted);')+'" onclick="window.dTC(\'s\',\'FreeNow\')">FreeNow</div>' +
                      '<div class="chip '+(window.dTSrc==='Inna'?'active':'')+'" style="'+chipStyle+(window.dTSrc==='Inna'?chipActBlue:'background:rgba(255,255,255,0.03); color:var(--muted);')+'" onclick="window.dTC(\'s\',\'Inna\')">Inna</div>';
            } else {
                ch1 = '<div class="chip '+(window.dTSrc==='Centrala'?'active':'')+'" style="'+chipStyle+(window.dTSrc==='Centrala'?chipActBlue:'background:rgba(255,255,255,0.03); color:var(--muted);')+'" onclick="window.dTC(\'s\',\'Centrala\')">Centrala</div>' +
                      '<div class="chip '+(window.dTSrc==='Postój'?'active':'')+'" style="'+chipStyle+(window.dTSrc==='Postój'?chipActBlue:'background:rgba(255,255,255,0.03); color:var(--muted);')+'" onclick="window.dTC(\'s\',\'Postój\')">Postój</div>' +
                      '<div class="chip '+(window.dTSrc==='Prywatny'?'active':'')+'" style="'+chipStyle+(window.dTSrc==='Prywatny'?chipActBlue:'background:rgba(255,255,255,0.03); color:var(--muted);')+'" onclick="window.dTC(\'s\',\'Prywatny\')">Prywatny</div>';
            }
            
            let otherSrcHtml = (d.plat === 'apps' && window.dTSrc === 'Inna') ? '<div class="inp-group" style="margin-top:12px;"><input type="text" id="dt-other-src" placeholder="Nazwa aplikacji..." style="background:rgba(0,0,0,0.3); border:1px solid rgba(255,255,255,0.1); border-radius:16px; color:#fff; padding:15px; font-size:0.85rem; text-align:center;" value="'+(window.dOtherSrc||'')+'" onchange="window.dOtherSrc=this.value"></div>' : '';
            
            let ch2 = '';
            if(d.plat === 'apps') {
                ch2 = '<div class="chip '+(window.dTPay==='Aplikacja'?'active':'')+'" style="'+chipStyle+(window.dTPay==='Aplikacja'?chipActBlue:'background:rgba(255,255,255,0.03); color:var(--muted);')+'" onclick="window.dTC(\'p\',\'Aplikacja\')">Aplikacja</div>' +
                      '<div class="chip '+(window.dTPay==='Gotówka'?'active':'')+'" style="'+chipStyle+(window.dTPay==='Gotówka'?chipActGreen:'background:rgba(255,255,255,0.03); color:var(--muted);')+'" onclick="window.dTC(\'p\',\'Gotówka\')">Gotówka</div>';
            } else {
                ch2 = '<div class="chip '+(window.dTPay==='Gotówka'?'active':'')+'" style="'+chipStyle+(window.dTPay==='Gotówka'?chipActGreen:'background:rgba(255,255,255,0.03); color:var(--muted);')+'" onclick="window.dTC(\'p\',\'Gotówka\')">Gotówka</div>' +
                      '<div class="chip '+(window.dTPay==='Karta'?'active':'')+'" style="'+chipStyle+(window.dTPay==='Karta'?chipActBlue:'background:rgba(255,255,255,0.03); color:var(--muted);')+'" onclick="window.dTC(\'p\',\'Karta\')">Karta</div>' +
                      '<div class="chip '+(window.dTPay==='Voucher'?'active':'')+'" style="'+chipStyle+(window.dTPay==='Voucher'?'background:rgba(168,85,247,0.15);color:#a855f7;border-color:rgba(168,85,247,0.4);':'background:rgba(255,255,255,0.03); color:var(--muted);')+'" onclick="window.dTC(\'p\',\'Voucher\')">Voucher</div>';
            }
            ...
            let stoperHtml = '<div style="background: rgba(59, 130, 246, 0.1); border: 1px solid rgba(59, 130, 246, 0.2); padding: 10px; border-radius: 12px; margin-bottom: 15px; text-align: center; font-size: 0.65rem; color: #0ea5e9; text-transform:uppercase; font-weight:800; letter-spacing:1px;">🔒 Śledzenie GPS wkrótce</div>';
            if(d.sh && d.sh.on) {
                if(d.liveRideStart) {
                    ...
                    stoperHtml += '<div class="panel" style="border-color:'+(isWaiting?'rgba(14,165,233,0.4)':'rgba(16,185,129,0.4)')+'; text-align:center; background:'+(isWaiting?'rgba(14,165,233,0.05)':'rgba(16,185,129,0.05)')+'; padding:15px; border-radius:20px; box-shadow:0 8px 25px rgba(0,0,0,0.3); margin-bottom:15px;">' +
                        ...
                        '<div style="display:flex; gap:10px;">' +
                            '<button class="btn" style="flex:1; padding:12px; font-size:0.8rem; border-radius:14px; font-weight:800; background:'+(isWaiting?'#10b981':'rgba(14,165,233,0.15)')+'; color:'+(isWaiting?'#000':'#0ea5e9')+'; border:none;" onclick="if(typeof window.toggleRideWait===\'function\') window.toggleRideWait()">'+(isWaiting?'▶ RUSZAMY':'⏳ POSTÓJ')+'</button>' +
                            '<button class="btn" style="flex:2; padding:12px; border-radius:14px; font-size:0.8rem; font-weight:800; background:rgba(239,68,68,0.15); color:#ef4444; border:none;" onclick="if(typeof window.stopLiveRide===\'function\') window.stopLiveRide()">🔴 ZAKOŃCZ</button>' +
                        '</div></div>';
                } else {
                    stoperHtml += '<div class="panel" style="border-color:rgba(16,185,129,0.2); background:rgba(16,185,129,0.05); text-align:center; padding:10px; border-radius:20px; margin-bottom:15px;">' +
                        '<button class="btn" style="background:#10b981; color:#000; font-size:0.85rem; font-weight:800; letter-spacing:1px; padding:15px; border-radius:14px; border:none; box-shadow:0 6px 15px rgba(16,185,129,0.2);" onclick="if(typeof window.startLiveRide===\'function\') window.startLiveRide()">🟢 ROZPOCZNIJ KURS (STOPER)</button></div>';
                }
            }
            ...
            if (d.sh && d.sh.on) {
                act += '<div class="dash-hero" style="padding-bottom:15px; border-bottom:1px dashed rgba(255,255,255,0.05); margin-bottom:15px;">' +
                    ...
                    '<div style="display:flex; justify-content:center; gap:10px; margin-top:20px; padding:0 5px;">' +
                        ...
                        '<div style="flex:1; display:flex; flex-direction:column; gap:8px;">' +
                            '<button style="flex:1; border-radius:14px; font-size:0.75rem; font-weight:800; background:'+(d.sh.sPS?'#10b981':'rgba(255,255,255,0.05)')+'; color:'+(d.sh.sPS?'#000':'#fff')+'; border:1px solid '+(d.sh.sPS?'#10b981':'rgba(255,255,255,0.1)')+'; cursor:pointer; outline:none;" onclick="if(typeof window.toggleShiftPause===\'function\') window.toggleShiftPause()">'+(d.sh.sPS?'▶ WZNÓW':'☕ PRZERWA')+'</button>' +
                            '<button style="flex:1; border-radius:14px; font-size:0.75rem; font-weight:800; background:rgba(239,68,68,0.15); color:#ef4444; border:none; cursor:pointer; outline:none;" onclick="if(typeof window.openEndShiftModal===\'function\') window.openEndShiftModal()">🔴 ZAKOŃCZ</button>' +
                        '</div>' +
                    '</div>' +
                '</div>';
                
                act += (d.sh.sPS ? '<div class="panel" style="border-color:rgba(245,158,11,0.3); text-align:center; padding:25px 15px; margin-top:10px; border-radius:20px; background:rgba(245,158,11,0.05);"><div style="font-size:2.5rem; margin-bottom:10px; animation: pulse 2s infinite;">☕</div><h2 style="color:#f59e0b; margin:0 0 15px 0; font-size:1.1rem;">ZMIANA WSTRZYMANA</h2><button class="btn" style="background:#10b981; color:#000; font-weight:800; padding:15px; border-radius:14px; border:none; font-size:1rem;" onclick="if(typeof window.toggleShiftPause===\'function\') window.toggleShiftPause()">▶ WZNÓW PRACĘ</button></div>' : stoperHtml);
                ...
                act += '<div class="panel" style="border:1px solid rgba(255,255,255,0.05); background:linear-gradient(145deg, #18181b, #09090b); padding:20px 15px; border-radius:24px; display:'+(d.sh.sPS?'none':'block')+'; box-shadow:0 10px 30px rgba(0,0,0,0.5); margin-bottom:20px;">' +
                    ...
                    '<button class="btn" style="background:#0ea5e9; color:#fff; padding:18px; border-radius:16px; font-weight:800; font-size:1rem; letter-spacing:0.5px; border:none; box-shadow:0 6px 20px rgba(14,165,233,0.3);" onclick="if(typeof window.dAddT===\'function\') window.dAddT()">DODAJ KURS</button>' +
                '</div>';
                ...
                '<div class="panel" style="border:1px solid rgba(255,255,255,0.05); background:linear-gradient(145deg, #18181b, #09090b); box-shadow:0 20px 40px rgba(0,0,0,0.6); padding:30px 20px; border-radius:24px; margin:0 15px;">' +
                    '<div style="background:rgba(0,0,0,0.4); border:1px inset rgba(255,255,255,0.05); border-radius:16px; padding:15px; margin-bottom:20px;">' +
                        '<input type="number" id="ds-o" value="'+((d.odo||0)>0?d.odo:'')+'" placeholder="ODO" style="width:100%; font-size:2.5rem; padding:0; text-align:center; background:transparent; border:none; color:#10b981; font-weight:800; outline:none;">' +
                    '</div>' +
                    '<button class="btn" style="background:#10b981; color:#000; font-size:1.1rem; font-weight:800; letter-spacing:0.5px; padding:18px; border-radius:16px; border:none; box-shadow:0 6px 20px rgba(16, 185, 129, 0.3); width:100%;" onclick="if(typeof window.dStartS===\'function\') window.dStartS()">ROZPOCZNIJ PRACĘ</button>' +
                '</div>';
                ...
                if(!window.dShowOff) {
                    ...
                    '<div class="panel" style="border:1px solid rgba(255,255,255,0.05); background:linear-gradient(145deg, #0f172a, #09090b); border-radius:24px; padding:20px 15px; margin:0 15px; animation:fadeIn 0.3s; box-shadow:0 10px 30px rgba(0,0,0,0.5);">' +
                        ...
                        '<div class="inp-group" style="margin-bottom:15px; background:rgba(0,0,0,0.4); border-radius:16px; padding:15px; border:1px inset rgba(255,255,255,0.05);">' +
                            ...
                            '<input type="number" id="dw-v" placeholder="0.00" style="width:100%; color:#0ea5e9; border:none; background:transparent; font-size:2.2rem; font-weight:800; text-align:center; outline:none; padding:0;">' +
                        '</div>' +
                        '<div class="inp-row" style="margin-bottom:15px; gap:10px;">' +
        ...
        if(t === 'stats') {
            ...
            if (d.plat === 'apps') {
                breakdownStatsHtml = '<div style="display:flex; justify-content:space-between; gap:10px; margin-top:20px; padding:0 15px; margin-bottom:20px;">' +
                    ...
                    '<div style="flex:1; background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.05); border-radius:16px; padding:15px 10px; text-align:center;"><span style="font-size:0.6rem; color:rgba(255,255,255,0.5); text-transform:uppercase; font-weight:800; letter-spacing:1px;">Uber</span><br><strong style="color:#fff; font-size:1.1rem; display:block; margin-top:6px;">'+Number(uberEarned).toFixed(2)+'</strong></div>' +
            ...
            if (availableCashForTransfer > 0) {
                transferButtonHtml = '<button class="btn btn-success" style="margin-top:20px; width:calc(100% - 30px); margin-left:auto; margin-right:auto; font-weight:800; background:#10b981; color:#000; box-shadow: 0 6px 20px rgba(16,185,129,0.3); padding:16px; border-radius:16px; border:none; outline:none; cursor:pointer;" onclick="if(typeof window.dTransferToHomeModal===\'function\') window.dTransferToHomeModal()"><span style="font-size:1.1rem; margin-right:8px;">💸</span> PRZELEJ DO BUDŻETU<br><small style="font-weight:600; font-size:0.7rem; color:rgba(0,0,0,0.6); display:block; margin-top:4px;">Gotówka: '+Number(availableCashForTransfer).toFixed(2)+' zł</small></button>';
            } else {
                transferButtonHtml = '<button class="btn" style="margin-top:20px; width:calc(100% - 30px); margin-left:auto; margin-right:auto; font-weight:700; background:rgba(255,255,255,0.03); color:rgba(255,255,255,0.3); border:1px solid rgba(255,255,255,0.05); padding:16px; border-radius:16px; outline:none;" disabled><span style="font-size:1.1rem; margin-right:8px;">✅</span> GOTÓWKA ROZLICZONA<br><small style="font-weight:600; font-size:0.7rem; display:block; margin-top:4px;">W domu: '+Number(totalTransferred).toFixed(2)+' zł</small></button>';
            }

            let proBannerHtml = '';
            if (d.plat === 'apps') {
                proBannerHtml = '<div style="margin: 20px 15px; padding: 20px; background: linear-gradient(135deg, rgba(168, 85, 247, 0.1), rgba(139, 92, 246, 0.05)); border: 1px solid rgba(168, 85, 247, 0.2); border-radius: 20px; cursor: pointer; text-align: center; box-shadow: 0 8px 25px rgba(0,0,0,0.2);" onclick="if(typeof window.sysAlert===\'function\') window.sysAlert(\'Inteligentny Asystent Zleceń (PRO)\', \'Wkrótce udostępnimy aplikację StyreOS PRO! Nakładka na ekran odczyta szczegóły zlecenia bezpośrednio z aplikacji partnerskiej i w ułamku sekundy pokaże Ci zysk na czysto na malutkim bąbelku, zanim zdążysz to zaakceptować! 🚀\', \'info\')">' +
                    '<div style="font-size: 1.8rem; margin-bottom: 8px;">🔮</div>' +
                    '<strong style="color: #c084fc; font-size: 0.85rem; display: block; text-transform: uppercase; letter-spacing:1px; font-weight:800;">Inteligentny Asystent</strong>' +
                    '<span style="font-size: 0.7rem; color: rgba(255,255,255,0.5); margin-top: 6px; display: block; line-height:1.4;">Pływająca nakładka opłacalności kursu - tylko w wersji PRO! Kliknij po info.</span>' +
                '</div>';
            } else {
                proBannerHtml = '<div style="margin: 20px 15px; padding: 20px; background: linear-gradient(135deg, rgba(14, 165, 233, 0.1), rgba(2, 132, 199, 0.05)); border: 1px solid rgba(14, 165, 233, 0.2); border-radius: 20px; cursor: pointer; text-align: center; box-shadow: 0 8px 25px rgba(0,0,0,0.2);" onclick="if(typeof window.sysAlert===\'function\') window.sysAlert(\'Integracja e-Kasy / RT3000 (PRO)\', \'W wersji StyreOS PRO wprowadzimy bezpośrednią integrację z systemami korporacyjnymi i kasami wirtualnymi (API). Zlecenia i e-paragony będą wpadać do aplikacji w 100% automatycznie! 🖨️☁️\', \'info\')">' +
                    '<div style="font-size: 1.8rem; margin-bottom: 8px;">📡</div>' +
                    '<strong style="color: #38bdf8; font-size: 0.85rem; display: block; text-transform: uppercase; letter-spacing:1px; font-weight:800;">Integracja e-Kasy</strong>' +
                    '<span style="font-size: 0.7rem; color: rgba(255,255,255,0.5); margin-top: 6px; display: block; line-height:1.4;">Automatyczne zaciąganie kursów (API) w wersji PRO. Kliknij po info.</span>' +
                '</div>';
            }

            let pAndLHtml = '<div class="panel" style="padding:25px 20px; margin:0 15px 15px; border-radius:24px; border:1px solid rgba(255,255,255,0.05); background:linear-gradient(145deg, #18181b, #09090b); box-shadow:0 10px 30px rgba(0,0,0,0.6);">' +
                '<div style="text-align:center; margin-bottom:20px;">' +
                    '<span style="font-size:0.7rem; color:rgba(255,255,255,0.4); font-weight:800; text-transform:uppercase; letter-spacing:1px;">Wodospad Finansowy (P&L)</span>' +
                '</div>' +
                '<div class="fin-row" style="font-size:0.8rem; margin-bottom:12px; border-bottom:1px solid rgba(255,255,255,0.05); padding-bottom:12px;"><span class="fin-label" style="color:#fff;">Utarg Brutto z aplikacji</span><strong class="fin-val" style="color:#10b981; font-size:0.95rem;">'+Number(g).toFixed(2)+' zł</strong></div>' +
                '<div class="fin-row" style="font-size:0.8rem; margin-bottom:12px; border-bottom:1px solid rgba(255,255,255,0.05); padding-bottom:12px;"><span class="fin-label" style="color:rgba(255,255,255,0.7);">Wydatki Zmienne (Garaż)</span><span class="fin-val" style="color:#ef4444;">-'+Number(ex).toFixed(2)+' zł</span></div>' +
                '<div class="fin-row" style="font-size:0.8rem; margin-bottom:12px; border-bottom:1px solid rgba(255,255,255,0.05); padding-bottom:12px;"><span class="fin-label" style="color:rgba(255,255,255,0.7);">Paliwo z tras</span><span class="fin-val" style="color:#f59e0b;">-'+Number(fc).toFixed(2)+' zł</span></div>' +
                (cf > 0 ? '<div class="fin-row" style="font-size:0.8rem; margin-bottom:12px; border-bottom:1px solid rgba(255,255,255,0.05); padding-bottom:12px;"><span class="fin-label" style="color:rgba(255,255,255,0.7);">Prowizja Terminala</span><span class="fin-val" style="color:#ef4444;">-'+Number(cf).toFixed(2)+' zł</span></div>' : '') +
                (vf > 0 ? '<div class="fin-row" style="font-size:0.8rem; margin-bottom:12px; border-bottom:1px solid rgba(255,255,255,0.05); padding-bottom:12px;"><span class="fin-label" style="color:rgba(255,255,255,0.7);">Prowizja Voucherów</span><span class="fin-val" style="color:#ef4444;">-'+Number(vf).toFixed(2)+' zł</span></div>' : '') +
                (pf > 0 ? '<div class="fin-row" style="font-size:0.8rem; margin-bottom:12px; border-bottom:1px solid rgba(255,255,255,0.05); padding-bottom:12px;"><span class="fin-label" style="color:rgba(255,255,255,0.7);">Prowizja Aplikacji</span><span class="fin-val" style="color:#ef4444;">-'+Number(pf).toFixed(2)+' zł</span></div>' : '') +
                '<div class="fin-row" style="font-size:0.8rem; margin-bottom:12px; border-bottom:1px solid rgba(255,255,255,0.05); padding-bottom:12px;"><span class="fin-label" style="color:rgba(255,255,255,0.7);">Podatek / VAT</span><span class="fin-val" style="color:#ef4444;">-'+Number(tx).toFixed(2)+' zł</span></div>' +
                '<div class="fin-row" style="align-items:flex-start; font-size:0.8rem; margin-bottom:15px;">' +
                    '<div style="display:flex; flex-direction:column; max-width:70%;"><span class="fin-label" style="color:rgba(255,255,255,0.9);">Odpisy Stałe '+(!window.db.drv.showFixed ? '(Wyłączone)' : '(Za '+daysToCharge+' dni)')+'</span></div>' +
                    '<span class="fin-val" style="color:#ef4444; padding-top:2px;">-'+Number(totalDynamicFix).toFixed(2)+' zł</span>' +
                '</div>' +
                '<div class="fin-row" style="background:rgba(0,0,0,0.4); padding:20px; border-radius:16px; border:1px inset rgba(255,255,255,0.05);">' +
                    '<span class="fin-label" style="color:#fff; font-size:0.95rem; font-weight:900; letter-spacing:1px;">WYNIK KOŃCOWY</span>' +
                    '<span class="fin-val" style="font-size:1.6rem; font-weight:900; letter-spacing:-0.5px; color:'+(n >= 0 ? '#10b981' : '#ef4444')+'">'+Number(n).toFixed(2)+' zł</span>' +
                '</div>' +
            '</div>';

            let historyLogHtml = '<div class="panel" style="margin:0 15px 15px; padding:20px 15px; border-radius:24px; border:1px solid rgba(255,255,255,0.05); background:linear-gradient(145deg, #18181b, #09090b); box-shadow:0 10px 30px rgba(0,0,0,0.6);">' +
                '<div style="text-align:center; margin-bottom:20px;">' +
                    '<span style="font-size:0.7rem; color:rgba(255,255,255,0.4); font-weight:800; text-transform:uppercase; letter-spacing:1px;">Dziennik Zmian</span>' +
                '</div>';
            
            if(fs.length > 0) {
                for(let i=0; i<fs.length; i++) {
                    let x = fs[i];
                    historyLogHtml += '<div class="log-item" style="border:none; border-left:3px solid '+(x.n >= 0 ? '#10b981' : '#ef4444')+'; flex-direction:column; align-items:flex-start; padding:15px; margin-bottom:12px; border-radius:16px; background:rgba(0,0,0,0.3);">' +
                        '<div style="display:flex; justify-content:space-between; width:100%; margin-bottom:8px;">' +
                            '<div><strong style="font-size:1rem; color:#fff; font-weight:700;">'+x.dt+'</strong></div>' +
                            '<strong style="color:'+(x.n >= 0 ? '#10b981' : '#ef4444')+'; font-size:1.05rem;">Netto: '+Number(x.n || 0).toFixed(2)+' zł</strong>' +
                        '</div>' +
                        '<div style="display:flex; justify-content:space-between; width:100%; align-items:center;">' +
                            '<span style="color:rgba(255,255,255,0.4); font-size:0.7rem;">Brutto: <span style="color:#fff;">'+Number(x.g || 0).toFixed(2)+' zł</span> | '+Number(x.k || 0).toFixed(1)+' km</span>' +
                            '<div style="display:flex; gap:8px;">' +
                                '<button class="btn" style="padding:8px 12px; background:rgba(255,255,255,0.05); border:none; border-radius:10px; font-size:0.75rem; cursor:pointer; outline:none;" onclick="if(typeof window.dEditHistory===\'function\') window.dEditHistory('+x.id+')">✏️</button>' +
                                '<button class="btn-danger" style="padding:8px 12px; border:none; border-radius:10px; font-weight:bold; background:rgba(239,68,68,0.15); color:#ef4444; font-size:0.75rem; cursor:pointer; outline:none;" onclick="if(typeof window.dDelHistory===\'function\') window.dDelHistory('+x.id+')">🗑️</button>' +
                            '</div>' +
                        '</div>' +
                    '</div>';
                }
            } else {
                historyLogHtml += '<div style="text-align:center;color:rgba(255,255,255,0.3);font-size:0.8rem;padding:25px 0; background:rgba(0,0,0,0.2); border-radius:16px; border:1px dashed rgba(255,255,255,0.05);">Brak zapisanych zmian w tym okresie.</div>';
            }
            historyLogHtml += '</div>';
            ...
            if(appContainer) {
                ...
                '<div class="mode-switch" style="margin:15px; border-radius:16px; padding:6px; background:rgba(0,0,0,0.4); border:1px solid rgba(255,255,255,0.05);">' +
                    '<div class="m-btn '+(fM==='today'?'active':'')+'" style="padding:10px; font-size:0.7rem; border-radius:12px; font-weight:700;" onclick="window.db.filter=\'today\'; window.render()">Dziś</div>' +
                    '<div class="m-btn '+(fM==='week'?'active':'')+'" style="padding:10px; font-size:0.7rem; border-radius:12px; font-weight:700;" onclick="window.db.filter=\'week\'; window.render()">Tydzień</div>' +
                    '<div class="m-btn '+(fM==='month'?'active':'')+'" style="padding:10px; font-size:0.7rem; border-radius:12px; font-weight:700;" onclick="window.db.filter=\'month\'; window.render()">Miesiąc</div>' +
                    '<div class="m-btn '+(fM==='all'?'active':'')+'" style="padding:10px; font-size:0.7rem; border-radius:12px; font-weight:700;" onclick="window.db.filter=\'all\'; window.render()">Wszystko</div>' +
                    '<div class="m-btn '+(fM==='custom'?'active':'')+'" style="padding:10px; font-size:0.7rem; border-radius:12px; font-weight:700;" onclick="window.db.filter=\'custom\'; window.render()">Własny</div>' +
                '</div>' +
                customDatesHtml +
                ...
                '<div class="dash-hero" style="padding-top:0; padding-bottom:20px; border-bottom:1px dashed rgba(255,255,255,0.05); margin-bottom:20px;">' +
                    '<p style="font-size:0.65rem; font-weight:800; color:rgba(255,255,255,0.4); letter-spacing:1.5px; text-transform:uppercase;">'+(window.db.drv.showFixed ? 'TWOJE PRAWDZIWE NETTO' : 'ZYSK Z KURSÓW (BEZ ZUS/AUTA)')+'</p>' +
                    '<h1 style="color:'+(n>=0?'#10b981':'#ef4444')+'; font-size:3.5rem; font-weight:900; letter-spacing:-1.5px; margin:0;">'+Number(n).toFixed(2)+' zł</h1>' +
                    transferButtonHtml +
                '</div>' +
                breakdownStatsHtml +
                ...
                '<div style="padding:0 15px; display:flex; flex-direction:column; gap:12px; margin-bottom:20px;">' +
                    '<div style="display:flex; gap:12px;">' +
                        '<div class="box" style="flex:1; padding:15px; border-radius:16px; background:linear-gradient(145deg, #18181b, #09090b); border:1px solid rgba(255,255,255,0.05);">' +
                            '<span style="color:#0ea5e9; font-size:0.65rem; text-transform:uppercase; font-weight:800; letter-spacing:0.5px;">Zysk / 1 km</span>' +
                            '<strong style="color:#fff; font-size:1.3rem; display:block; margin-top:6px;">'+Number(rKm).toFixed(2)+' zł</strong>' +
                        '</div>' +
                        '<div class="box" style="flex:1; padding:15px; border-radius:16px; background:linear-gradient(145deg, #18181b, #09090b); border:1px solid rgba(255,255,255,0.05);">' +
                            '<span style="color:#d946ef; font-size:0.65rem; text-transform:uppercase; font-weight:800; letter-spacing:0.5px;">Zysk / 1 h</span>' +
                            '<strong style="color:#fff; font-size:1.3rem; display:block; margin-top:6px;">'+Number(rHr).toFixed(2)+' zł</strong>' +
                        '</div>' +
                    '</div>' +
                    '<div style="display:flex; gap:12px;">' +
                        '<div class="box" style="flex:1; padding:15px; border-radius:16px; background:linear-gradient(145deg, #18181b, #09090b); border:1px solid rgba(255,255,255,0.05);">' +
                            '<span style="color:#10b981; font-size:0.65rem; text-transform:uppercase; font-weight:800; letter-spacing:0.5px;">Dni w pracy</span>' +
                            '<strong style="color:#fff; font-size:1.3rem; display:block; margin-top:6px;">'+daysToCharge+' dni</strong>' +
                        '</div>' +
                        '<div class="box" style="flex:1; padding:15px; border-radius:16px; background:linear-gradient(145deg, #18181b, #09090b); border:1px solid rgba(255,255,255,0.05);">' +
                            '<span style="color:#f59e0b; font-size:0.65rem; text-transform:uppercase; font-weight:800; letter-spacing:0.5px;">Czas za kółkiem</span>' +
                            '<strong style="color:#fff; font-size:1.3rem; display:block; margin-top:6px;">'+Number(th).toFixed(1)+' h</strong>' +
                        '</div>' +
                    '</div>' +
                    '<div style="display:flex; gap:12px;">' +
                        '<div class="box" style="flex:1; border:1px solid rgba(16,185,129,0.15); background:rgba(16,185,129,0.05); padding:15px; border-radius:16px;">' +
                            '<span style="color:#10b981; font-size:0.65rem; text-transform:uppercase; font-weight:800; letter-spacing:0.5px;">KM z Klientem</span>' +
                            '<strong style="color:#fff; font-size:1.3rem; display:block; margin-top:6px;">'+Number(pkSum).toFixed(1)+' km</strong>' +
                        '</div>' +
                        '<div class="box" style="flex:1; border:1px solid rgba(239,68,68,0.15); background:rgba(239,68,68,0.05); padding:15px; border-radius:16px;">' +
                            '<span style="color:#ef4444; font-size:0.65rem; text-transform:uppercase; font-weight:800; letter-spacing:0.5px;">Puste (Dojazdy)</span>' +
                            '<strong style="color:#fff; font-size:1.3rem; display:block; margin-top:6px;">'+Number(emptyKSum).toFixed(1)+' km</strong>' +
                        '</div>' +
                    '</div>' +
                '</div>' +
