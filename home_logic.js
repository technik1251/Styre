...
window.hGetBal = function() {
    window.hInitDb();
    let b = {}; window.db.home.accs.forEach(a => b[a.id] = a.startBal || 0);
...
window.hSyncSchedule = function() {
    ...
    for(let i=0; i<12; i++) {
        ...
        if(window.db.home.recurring) window.db.home.recurring.forEach(r => {
            let dObj = new Date(curD.getFullYear(), curD.getMonth(), r.day || 1, 12, 0, 0);
            if(dObj > n) {
                window.db.home.trans.push({ id: 'rec_'+r.id+'_'+i, type: r.t, cat: r.c, acc: r.a, d: r.n, v: parseFloat(r.v), who: 'System', dt: dObj.toLocaleDateString('pl-PL'), rD: dObj.toISOString(), isPlanned: true, recId: r.id });
            }
        });
    }

    // 2. Kredyty i Karty Kredytowe
    if(window.db.home.loans) window.db.home.loans.filter(l => !l.isClosed).forEach(l => {
            ...
            if (kap > 0) {
                ...
                for(let i=0; i<12; i++) {
                    ...
                    if(dObj > n || (i===0 && dObj.getDate() >= n.getDate())) {
                        window.db.home.trans.push({ id: 'loan_'+l.id+'_'+i, type: 'exp', cat: 'Kredyt / Leasing', acc: l.accId || window.db.home.accs[0].id, d: 'Karta: ' + l.n, v: plannedV, who: 'System', dt: dObj.toLocaleDateString('pl-PL'), rD: dObj.toISOString(), isPlanned: true, loanId: l.id });
                        break; 
                    }
                }
            }
        ...
        // Logika dla KREDYTÓW / LEASINGÓW
            ...
            for(let i=0; i<24 && generated < instLeft; i++) {
                ...
                if(dObj > n || (generated===0 && dObj.getDate() >= n.getDate())) {
                     window.db.home.trans.push({ id: 'loan_'+l.id+'_'+i, type: 'exp', cat: 'Kredyt / Leasing', acc: l.accId || window.db.home.accs[0].id, d: 'Rata: ' + l.n, v: parseFloat(l.rata), who: 'System', dt: dObj.toLocaleDateString('pl-PL'), rD: dObj.toISOString(), isPlanned: true, loanId: l.id });
                }
                generated++;
            }
        }
    ...
    if(window.db.home.recurring) {
        window.db.home.recurring.forEach(r => {
            if(r.lastBooked !== cM && cD >= (r.day || 1)) {
                ...
                window.db.home.trans.push({ id: Date.now() + Math.random(), type: r.t, cat: r.c, acc: r.a, d: r.n + ' (Automat)', v: parseFloat(r.v), who: 'System', dt: dObj.toLocaleDateString('pl-PL'), rD: dObj.toISOString(), isPlanned: false });
