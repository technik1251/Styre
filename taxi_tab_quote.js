// ==========================================
// PLIK: taxi_tab_quote.js - Asystent Wyceny i Prawdziwy Taksometr Mapowy
// ==========================================
...
window.rDrvQuote = function(d, t, nav, hdr) {
    try {
        ...
        act += '<input type="text" id="qa-end" placeholder="np. Powstańców Warszawy 1, Szczecin" style="width:100%; background:rgba(239,68,68,0.05); border:1px solid rgba(239,68,68,0.2); color:#fff; padding:18px; border-radius:16px; font-size:1.05rem; font-weight:700; outline:none; box-sizing:border-box;">';
        act += '</div>';
        ...
        // --- UKRYTA MAPA I WYNIKI (POJAWIA SIĘ PO WYZNACZENIU) ---
...
window.qFindRoute = async function() {
    ...
    if(!startAddr || !endAddr) {
        if(window.sysAlert) window.sysAlert("Błąd", "Wpisz adres początkowy i docelowy (najlepiej z miastem, np. 'Wiejska 1, Szczecin').", "error");
        return;
    }
    ...
    try {
        ...
        if(!startCoords || !endCoords) {
            throw new Error("Nie znaleziono jednego z adresów. Spróbuj dodać nazwę miasta.");
        }

        let routeData = await window.qRoute(startCoords, endCoords);
        if(!routeData || !routeData.routes || routeData.routes.length === 0) {
            throw new Error("Nie można wyznaczyć trasy dla tych punktów.");
        }
