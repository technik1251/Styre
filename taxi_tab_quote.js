...
window.qFindRoute = async function() {
    ...
    try {
        // Geokodowanie (Adres -> Współrzędne)
        ...
        // Routing OSRM (Wyznaczanie trasy)
        ...
        window.qCurrentDist = route.distance / 1000; // km
        window.qCurrentTime = route.duration / 60; // minuty
        ...
        window.qMapInstance.invalidateSize(); // Ważne, żeby mapa się narysowała w pokazanym divie
...
window.qUpdatePrice = function() {
    ...
    // Pobierz wartość suwaka (0-100%) - ile procent trasy jest w Strefie 1 (Miasto)
    ...
    // Stawki w zależności od wybranej taryfy (dzień/noc)
    let rateCity = window.qTariffMode === 'day' ? q.t1 : q.t2;
    let rateOutside = window.qTariffMode === 'day' ? q.t3 : q.t4;

    // Kalkulacja: Opłata startowa + Dystans Miejski + Dystans Zamiejski
    let price = q.s + (distCity * rateCity) + (distOutside * rateOutside);
    
    // Dodanie szacowanego czasu postoju (zakładamy 1 min postoju na każde 5 min jazdy przez ruch)
